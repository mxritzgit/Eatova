...
void main() {
  testWidgets(
    'recipes open the meal planner bound to the same signed-in store',
    (tester) async {
      await withClock(Clock.fixed(_now), () async {
        ...
        final planner = tester.widget<MealPlanScreen>(
  ...
  testWidgets(
    ...
    (tester) async {
      await withClock(Clock.fixed(_now), () async {
        ...
        expect(
          find.byKey(const ValueKey('coach-brief-scroll')),
          findsOneWidget,
        );
        ...
        expect(
          coach.selectedPlanForCoach?.workouts.single.exercises.single.reps,
          8,
        );
        expect(
  ...
  testWidgets(
    'actual workout reaches history and Last time through shell callbacks',
    (tester) async {
      await withClock(Clock.fixed(_now), () async {
        final server = FixlaufServer()
          ..profileRow = serverProfileRow(completedProfile);
