...
void main() {
  ...
  test('Offline-Log: KEIN Rollback, Op persistiert in der Outbox, '
      'dezenter Hinweis statt rotem Sync-Fehler', () async {
    final s = setup();
    s.server.offline = true;
    await boot(s.store);

    final id = await s.store.addResultToDailyTotal(mealResult('Offline-Bowl'));
    await settle();
    ...
    expect(
      s.snacks.offlineHints.single,
      'Offline — wird synchronisiert, sobald du wieder online bist.',
    );
    final ton = s
        .snacks
        .tones[s.snacks.messages.indexWhere((m) => m.startsWith('Offline'))];
    expect(ton, isNot(SnackTone.error), reason: 'kein Rot-Alarm');
  });

  test('Server-Fehler (500) statt Netz weg: Op queued wie gehabt, aber der '
  ...
  test('Luecke E: ein zugestelltes Rezept meldet delivered', () async {
    ...
    expect(
      await s.store.createUserRecipe(userRecipe('user_ok')),
      SyncDelivery.delivered,
    );
    expect(s.server.recipeRows.keys, contains('user_ok'));
  });

  test('Luecke E: offline eingereiht meldet queuedOffline — und der Store '
    ...
    expect(
      await s.store.createUserRecipe(userRecipe('user_offline')),
    ...
    expect(
      ...
      isEmpty,
      reason:
          'zwei Meldungen hintereinander („gespeichert." und gleich '
  ...
  test(
    'Luecke E: eine Server-Ablehnung meldet queuedRetry, nicht Offline',
    () async {
      final s = setup();
      await boot(s.store);
      ...
      expect(
        await s.store.createUserRecipe(userRecipe('user_500')),
  ...
  test('Luecke E: die Loeschung meldet ihren Ausgang genauso', () async {
    ...
    expect(
      await s.store.deleteUserRecipe('user_weg'),
      SyncDelivery.queuedOffline,
    );
    expect(s.snacks.messages, isEmpty);
  });

  test('Luecke E: ein haengender Write blockiert die Rueckmeldung nicht ewig — '
    ...
    expect(
      ...
      reason:
          'die Meldung darf nur behaupten, was auch stimmt: die Op liegt '
  ...
  test('Luecke D: offline geaendertes Gewicht steht nach dem naechsten '
    ...
    expect(
      b.store.profile.weightKg,
      84,
      reason:
          'die Serverzeile darf eine pendende Profil-Aenderung nicht '
          'ueberschreiben',
    );
    expect(b.store.profile.dailyKcalGoal, 1900);
    expect(
      b.server.profileRow!['weight_kg'],
      84,
      reason: 'und sie muss zugestellt werden, nicht nur lokal ueberleben',
    );
    expect(
      (await b.cache.readProfile())!.weightKg,
      84,
      reason:
  ...
  test(
    ...
    () async {
      ...
      expect(
        b.store.pendingOutbox.map((o) => o.kind),
        contains(SyncOpKind.profileUpsert),
        reason:
            'die Zustellung steht weiterhin aus und muss die Sitzung '
            'ueberleben',
      );
    },
  );
  ...
  test(
    'Luecke D: zwei Offline-Aenderungen bleiben immutable FIFO — das Profil ist '
    'eine einzelne Zeile, die letzte Aenderung gewinnt',
    () async {
      final a = setup();
      a.server.profileRow = serverProfileRow(testProfile(weightKg: 80));
      await boot(a.store);
      ...
      expect(
        ...
        hasLength(2),
        reason:
            'Gestartete Payloads bleiben unveraendert und werden FIFO zugestellt',
      );
      expect(profilOps.last.profile!.weightKg, 86);
      expect(profilOps.last.profile!.dailyKcalGoal, 2000);
      ...
      int profilPosts() => a.server.operations('profileUpsert').length;
      ...
      expect(
        profilPosts() - vorReplay,
        2,
        reason: 'Immutable Vorganger wird vor der neueren Fassung zugestellt',
      );
  ...
  test(
    ...
    () async {
      ...
      expect(
        a.snacks.offlineHints.single,
        'Offline — wird synchronisiert, sobald du wieder online bist.',
        reason:
            'die alte Meldung bat den Nutzer, spaeter selbst erneut zu '
            'speichern — das waere jetzt gelogen',
      );
      expect(
        a.snacks.tones,
        isNot(contains(SnackTone.error)),
        reason: 'eine Warteschlange ist kein Fehler',
      );
    },
  );

  test('Luecke D: ein offline abgeschlossenes Onboarding ueberlebt den '
    ...
    expect(
      b.store.needsOnboarding,
      isFalse,
      reason:
          'die alte Bootstrap-Zeile schickte den Nutzer erneut durchs '
  ...
  test('Luecke D: eine pendende Profil-Op IST eine echte Quelle — sonst '
    ...
    await seedRawOutbox(kv, <Map<String, dynamic>>[
      SyncOp.profileUpsert(
        testProfile(weightKg: 91, dailyKcalGoal: 1800),
      ).toJson(),
    ]);
    ...
    expect(
      ...
      isNotNull,
      reason:
          'ohne diese Zuordnung schrieb applySettings gar nichts mehr — '
  ...
  test('Luecke D: auch die Diaet-Praeferenz ueberlebt — sie ist Teil derselben '
      'Profilzeile', () async {
    final kv = InMemoryKeyValueStore();
    final a = setup(kv: kv);
