...
void main() {
  ...
  test('Luecke A: ein im Flugmodus angelegtes Rezept ueberlebt den Kaltstart — '
    ...
    await a.store.createUserRecipe(
      userRecipe('user_flug', title: 'Flugmodus-Bowl'),
    );
    await settle();
    expect(
      a.store.userRecipes.map((r) => r.slug),
      contains('user_flug'),
    ...
    expect(
      b.store.userRecipes.map((r) => r.slug),
      contains('user_flug'),
      reason:
          'ohne Cache-Slot war das Rezept nach dem Kaltstart weg: die '
          'Outbox war das EINZIGE Netz und hatte ihre Schuldigkeit getan',
    );
  });

  test('Luecke A+B: haengt der Rezept-Write (Supabase-Aufrufe tragen kein '
    ...
    await a.store.createUserRecipe(userRecipe('user_haenger'));
    await settle();
    // Gap B: with no answer neither callback fires, so the op must pre-exist.
    expect(
      a.store.pendingOutbox.map((o) => o.entityKey),
      contains('recipe:user_haenger'),
      reason:
          'die Op darf nicht erst im Fehler-Callback entstehen — der '
          'kommt hier nie',
    );
    a.store.flushPendingWrites();
    await settle();
    ...
    expect(
      ...
      contains('user_haenger'),
      reason:
          'zwischen Tap und (ausbleibendem) Fehler existierte das '
          'Rezept nur im RAM',
    );
  });

  test('Luecke A: der Kaltstart ohne Netz zeigt die BESTEHENDEN Eigen-Rezepte '
    ...
    expect(
      ...
      containsAll(<String>['user_alt_1', 'user_alt_2']),
      reason:
          'der Boot-Snapshot muss die Rezepte mitnehmen — sonst zeigt '
          'jeder Start im Flugmodus eine leere Eigen-Rezept-Liste',
    );
  });

  test('Luecke A: eine Offline-Loeschung ueberlebt den Kaltstart und reisst '
    ...
    a.server.offline = true;
    await a.store.deleteUserRecipe('user_weg');
    await settle();
    a.store.flushPendingWrites();
    await settle();
  ...
  test(
    ...
    () async {
      ...
      await seed.writeUserRecipes(<FitnessRecipe>[
        userRecipe('user_nur_lokal'),
      ]);
      ...
      expect(
        s.store.userRecipes.map((r) => r.slug),
        containsAll(<String>['user_nur_lokal', 'user_server']),
        reason:
            'der Server-Load hat den lokalen Stand frueher restlos '
            'ueberschrieben',
      );
      expect(
        (await s.cache.readUserRecipes())!.map((r) => r.slug),
        contains('user_nur_lokal'),
        reason:
            'der Boot-Snapshot darf den Verlust nicht auch noch '
            'festschreiben',
      );
    },
  );

  test('Luecke C: ein lokal geloeschtes Rezept kommt NICHT zurueck, obwohl der '
    ...
    await seed.writeUserRecipes(<FitnessRecipe>[
      userRecipe('user_bleibt'),
      userRecipe('user_weg'),
    ]);
    await seedRawOutbox(kv, [
      SyncOp.recipeDelete('user_weg', expectedRevision: 2).toJson(),
    ]);
    ...
    expect(
      ...
      reason:
          'der Merge muss vom LEBENDEN Stand ausgehen (Cache + bereits '
          'angewandte Ops), nicht vom rohen Cache-Blob — der kennt die '
          'Loeschung noch nicht',
    );
    expect(
      s.server.recipeRows.keys,
      isNot(contains('user_weg')),
  ...
  test('Fehlerbild des Nutzers: Flugmodus -> Rezept angelegt -> App zu -> '
    ...
    a.server.hangRecipeWrites = true;
    await a.store.createUserRecipe(
      userRecipe('user_flugmodus', title: 'Flug-Bowl'),
    );
    await settle();
    expect(
      a.store.userRecipes.map((r) => r.slug),
      contains('user_flugmodus'),
    ...
    expect(
      ...
      contains('user_flugmodus'),
      reason:
          'genau hier war das Rezept weg: der erfolgreiche Server-Load '
  ...
  test('Gegenprobe 1 — der gemeldete Fall wortgetreu: Flugmodus AN, Rezept '
    ...
    a.server.offline = true;
    final ausgang = await a.store.createUserRecipe(
      userRecipe('user_gemeldet', title: 'Bowl'),
    );
    ...
    expect(
      b.server.recipeRows.keys,
      contains('user_gemeldet'),
      reason:
          'sichtbar reicht nicht — es muss auch ankommen, sonst haengt '
  ...
  test(
    ...
    () async {
      ...
      expect(
        ...
        isNot(contains('user_weg')),
        reason:
            'eine Loeschung, die wiederkommt, ist derselbe Vertrauens'
      ...
      expect(
        ...
        isNot(contains('user_weg')),
        reason:
            'lokal weg reicht nicht — sonst kommt es auf dem naechsten '
            'Geraet zurueck',
      );
      expect(
        (await b.cache.readUserRecipes())!.map((r) => r.slug),
  ...
  test('Gegenprobe 3 — offline ANGELEGT und offline wieder GELOESCHT: nach der '
    ...
    expect(
      s.server.recipeRows.keys,
      isNot(contains('user_kurz')),
      reason:
          'insert -> delete muss den Replay in dieser Reihenfolge '
  ...
  test(
    ...
    () async {
      ...
      expect(
        b.store.userRecipes.map((r) => r.slug),
        contains('user_zweimal'),
        reason: 'Vorbedingung: der zweite Start ohne Netz zeigt es',
      );
      expect(
        b.store.pendingOutbox.map((o) => o.entityKey),
      ...
      expect(
        b.server.requests.where(
          (r) =>
              r.url.path.endsWith('/rpc/apply_sync_operation') &&
              (jsonDecode(r.body) as Map)['p_kind'] == 'recipeUpsert',
        ),
        hasLength(1),
        reason:
            'der Cache-Stand darf keine zweite Zustellung ausloesen — '
  ...
  test('Gegenprobe 5 — ZWEI Rezepte offline: beide ueberleben den Kaltstart, '
    ...
    await settle();
    expect(a.store.userRecipes.map((r) => r.slug).toList(), <String>[
      'user_zwei',
      'user_eins',
    ...
    await boot(b.store);
    expect(
      b.store.userRecipes.map((r) => r.slug).toList(),
      <String>['user_zwei', 'user_eins'],
      reason:
          'Cache-Blob und Op-Ueberlagerung duerfen die Liste nicht '
  ...
  test('Gegenprobe 6 — der Server ANTWORTET beim Replay mit 500: das Rezept '
  ...
  test(
    'Without durable storage recipe save fails without publishing success',
    () async {
      final s = setupOhneCache();
      await boot(s.store);
      final requestsBefore = s.server.requests.length;
      await expectLater(
        s.store.createUserRecipe(userRecipe('user_ohne_cache')),
        throwsStateError,
      );
      ...
      expect(
        s.server.requests.length,
        requestsBefore,
        reason: 'No network write may precede the durable local commit',
      );
    },
  );

  test('Gegenprobe 8 — Nutzerwechsel: die Eigen-Rezepte von A tauchen bei B '
    ...
    expect(
      kv.snapshot.keys,
      contains('eatova.v1.user_recipes.user-a'),
    ...
    expect(
      kv.snapshot.keys,
      isNot(contains('eatova.v1.user_recipes.user-a')),
      reason:
          'Zutaten und Mengen sind Nutzerinhalt — der Slot faellt beim '
    ...
    expect(
      ...
      isEmpty,
      reason:
          'jeder Slot-Name traegt die User-ID — B liest seinen eigenen, '
          'leeren Namensraum',
    );
    expect(
  ...
  test('Gegenprobe 9 — EIN Netz faellt aus: der Outbox-Slot ist beim Lesen '
    ...
    final b = setup(injizierterCache: OutboxLesefehlerCache(kv, 'user-outbox'));
    await boot(b.store);
    ...
    expect(
      ...
      contains('user_nur_cache'),
      reason:
          'und der Boot-Snapshot darf den Verlust nicht auch noch '
          'festschreiben',
    );
  });
  ...
  test('P3-04b: userRecipesAuthoritative erst nach BEANTWORTETEM Rezept-Load — '
    ...
    expect(
      ...
      isFalse,
      reason:
          'Der Rezept-Load hat nicht geantwortet — diese leere Liste ist '
          'keine Aussage darueber, welche Rezepte es gibt.',
    );
    expect(
      s.server.requests.map((r) => r.url.path),
      contains('/rest/v1/profiles'),
      reason:
          'und der Boot als ganzes lief durch: das Flag steht fuer die '
    ...
    expect(
      ...
      isTrue,
      reason:
          'Nach der Antwort ist die Liste vollstaendig — ab hier darf '
