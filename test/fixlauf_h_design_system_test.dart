...
void main() {
  ...
  group('F8-02 Button-Themes', () {
    for (final p in _paletten.entries) {
      final brightness = p.key == 'hell' ? Brightness.light : Brightness.dark;
      final t = p.value;

      test('${p.key}: TextButton-Label (leises ink) auf surf >= 4.5:1', () {
      ...
      test('${p.key}: FilledButton-Flaeche (ink) auf surf >= 3:1, Text bg',
          () {
        final theme = buildEatovaTheme(brightness);
      ...
      test('${p.key}: OutlinedButton traegt line-Rand, ink-Text, 48 px', () {
        final theme = buildEatovaTheme(brightness);
    ...
    testWidgets('ein FilledButton im Dialog bleibt 48, nicht 54', (tester) async {
      await tester.pumpWidget(
        designHarness(
          Row(
            children: <Widget>[
  ...
  group('F8-03 inputDecorationTheme', () {
    for (final p in _paletten.entries) {
      ...
      test('${p.key}: keine sichtbare Linie in irgendeinem Zustand', () {
        final deco = buildEatovaTheme(brightness).inputDecorationTheme;
      ...
      test('${p.key}: Fokus hellt die Flaeche auf, Fehler toent danger', () {
        final deco = buildEatovaTheme(brightness).inputDecorationTheme;
      ...
      test('${p.key}: Hint- und Wert-Text bleiben auf allen drei Flaechen AA',
          () {
      ...
      test('${p.key}: Kapsel-Fuellungen sind weder surf noch surf2 noch bg',
          () {
        for (final f in <Color>[t.field, t.fieldFocus, t.fieldError]) {
      ...
      test('${p.key}: Ruhe-Kapsel hebt sich von Karte und Sheet ab', () {
      ...
      test('${p.key}: Fokus-Kapsel bleibt auf Karte und Sheet sichtbar', () {
  ...
  group('F8-03 FieldCapsule', () {
    ...
    testWidgets('folgt dem FocusNode: field -> fieldFocus, ohne Rand',
        (tester) async {
      ...
      expect(deco(tester).color, AppTokens.light.field);
      expect(deco(tester).border, isNull);
      expect(deco(tester).boxShadow, isNotEmpty);

      await tester.tap(find.byType(TextField));
      await tester.pumpAndSettle();
      expect(deco(tester).color, AppTokens.light.fieldFocus);
    ...
    testWidgets('focused/error/shape/shadow steuern die Kapsel direkt',
        (tester) async {
      await tester.pumpWidget(
        designHarness(
          const FieldCapsule(
      ...
      expect(deco(tester).color, AppTokens.light.fieldError,
          reason: 'Fehler schlaegt Fokus');
    });
  });

  group('F8-03 SheetField', () {
    testWidgets('ist rahmenlos, weich schattiert und rControl-rund',
        (tester) async {
      await tester.pumpWidget(
    ...
    testWidgets('Fokus hellt die Kapsel auf statt einen Ring zu zeichnen',
        (tester) async {
      await tester.pumpWidget(
        designHarness(const SheetField(label: 'Name', hint: 'hint')),
      );
      await tester.tap(find.byType(TextField));
      await tester.pumpAndSettle();
      final deco = _fieldDecoration(tester);
      expect(deco.color, AppTokens.light.fieldFocus);
      expect(deco.border, isNull);
    });

    testWidgets('Fehler toent die Flaeche, kein roter Rand', (tester) async {
      await tester.pumpWidget(
        designHarness(
      ...
      final deco = _fieldDecoration(tester);
      expect(deco.border, isNull);
      expect(deco.color, AppTokens.light.fieldError);
    ...
    testWidgets('shape.pill rundet voll', (tester) async {
      ...
      expect(_fieldDecoration(tester).borderRadius, BorderRadius.circular(rPill));
    });

    testWidgets('label ist optional (Suchfelder)', (tester) async {
      await tester.pumpWidget(
        designHarness(const SheetField(hint: 'Suchen')),
      );
    ...
    testWidgets('rendert in hell und dunkel mit allen Zustaenden',
        (tester) async {
  ...
  group('F8-10 SheetScaffold-Aktion', () {
    testWidgets('ist ein PrimaryActionButton mit ink-Flaeche', (tester) async {
      await tester.pumpWidget(
        designHarness(
          SheetScaffold(
            title: 'T',
      ...
      expect(material.color, AppTokens.light.ink);
    ...
    for (final p in _paletten.entries) {
      final brightness = p.key == 'hell' ? Brightness.light : Brightness.dark;
      final t = p.value;
      testWidgets('${p.key}: deaktivierter PrimaryActionButton sieht anders '
          'aus und bleibt als Flaeche erkennbar', (tester) async {
        Color fillOf() => tester
            .widget<Material>(
              find
                  .descendant(
        ...
        await tester.pumpWidget(
          designHarness(
            PrimaryActionButton(label: 'Weiter', onTap: () {}),
  ...
  group('F8-04 AppToggle AUS', () {
    for (final p in _paletten.entries) {
      ...
      testWidgets('${p.key}: Knopf und Spur sind gegen surf >= 3:1 erkennbar',
          (tester) async {
        await tester.pumpWidget(
          designHarness(
  ...
  group('F8-08 Makro-Farben', () {
    for (final p in _paletten.entries) {
      final t = p.value;
      test('${p.key}: jede Makro-Farbe erreicht 3:1 auf surf', () {
        for (final paar in <(String, Color)>[
          ...
          expect(_kontrast(paar.$2, t.surf), greaterThanOrEqualTo(3.0),
              reason: '${p.key}: ${paar.$1}-Balken auf der Karte');
        }
      });

      test('${p.key}: readableOnTint(carbs) bleibt AA auf seiner Tint', () {
  ...
  group('F8-09 Textskalierung', () {
    testWidgets('MealAvatar nutzt keine FittedBox mehr und waechst mit',
        (tester) async {
      await tester.pumpWidget(
        designHarness(MealAvatar(letter: 'F', color: AppTokens.light.carbs)),
      );
      expect(
        find.descendant(
  ...
  group('F8-06 showEatovaSheet', () {
    ...
    testWidgets('Standard: Handle an, Scrim = t.scrim, Handle 40x4',
        (tester) async {
      await tester.pumpWidget(
        opener((c) => showEatovaSheet<void>(c, const Text('Inhalt'))),
      );
    ...
    testWidgets('SheetHandle: 40x4, line, ohne Semantik', (tester) async {
      final handle = tester.ensureSemantics();
      await tester.pumpWidget(designHarness(const SheetHandle()));
      ...
      expect((bar.decoration! as BoxDecoration).color, AppTokens.light.line);
      expect(
        find.descendant(
          of: find.byType(SheetHandle),
    ...
    test('scrim ist ein Token und in beiden Modi halbtransparent', () {
      for (final p in _paletten.entries) {
        expect(p.value.scrim.a, inExclusiveRange(0.3, 0.8),
            reason: '${p.key}: Scrim muss den Hintergrund noch zeigen');
      }
    });
  });
  ...
  group('F8-07 FilterChipPill', () {
        ...
        designHarness(
          Align(
            child: FilterChipPill(
              ...
              dotColor: AppTokens.light.carbs,
            ),
          ),
        ),
      );
    ...
    testWidgets('Selektion ist immer forest + onForest, Radius rChip',
        (tester) async {
          ...
          designHarness(
            Align(
              child: FilterChipPill(
                ...
                selected: true,
                tone: tone,
                dotColor: AppTokens.light.snack,
    ...
    testWidgets('alle Varianten ueberstehen hell/dunkel und textScaler 2.0',
        (tester) async {
      pinPhoneViewport(tester);
      Widget chips() => Wrap(
            spacing: 8,
      ...
      await expectRendersInBothBrightnesses(tester, chips, scrollable: true);
      await expectSurvivesTextScale(tester, chips());
    });
  });
  ...
  group('F8-11 Radius-Skala', () {
    testWidgets('AppCard rundet mit rCard', (tester) async {
      await tester.pumpWidget(designHarness(const AppCard(child: Text('K'))));
      expect(
    ...
    testWidgets('PrimaryActionButton rundet mit rButton', (tester) async {
      await tester.pumpWidget(
        designHarness(const PrimaryActionButton(label: 'Weiter')),
      );
      final material = tester.widget<Material>(
        find
            .descendant(
