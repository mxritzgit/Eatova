...
| Flutter | Flutter **3.47.2 / Dart 3.13.2**; strikte Analyse grün. Gesamtlauf: **4551 bestanden, vier Integrationsfehler entdeckt**. Fehlende Schema-Erwartungen und Client-Fehlercodes anschließend korrigiert; alle **228** betroffenen gemeinsamen Regressionen grün. | Die vollständige Suite wird auf dem finalen PR-Stand erneut durch CI geprüft. Coverage-Floor bleibt **88%**; kein Absenken oder Ausblenden von Sicherheitsprüfungen. |
...
**Backend-Rollout dieser Runde:** Die drei neuen Migrationen sowie `coach-chat`, `analyze-meal` und `search-key` werden erst nach grüner geschützter PR-CI bereitgestellt. Vorherige aktive Quellen sind für den Rollback gesichert. Der noch aktive Stand ist v47/v30/v9; neue Migrationsdateien allein sind kein Live-Nachweis. Die vorbereitete Datenschutzkorrektur wird zusammen mit den neuen technischen Tageszählern veröffentlicht. Push, Merge und diese Veröffentlichungen werden unten nach tatsächlichem Abschluss getrennt eingetragen.
...
|---|---|---|---|---|---|
| A01 | RLS-Abdeckung aller erreichbaren Tabellen | ERFÜLLT | P1 | LIVE: 16/16 bisherige App-Tabellen mit RLS; CODE/TEST: auch drei neue Budgettabellen mit RLS, kombinierter PG17.6-Replay. [Schema](supabase/SCHEMA_STATE.md), [Budgetmigration](supabase/migrations/20260915091000_ai_provider_budgets.sql). | Nach Rollout alle 19 App-Tabellen einschließlich API-exponierter Schemas erneut katalogseitig vergleichen. |
| A02 | CRUD, USING und WITH CHECK | ERFÜLLT | P1 | LIVE: alle 34 bisherigen Policies mit lokalem Replay abgeglichen; TEST prüft erlaubte eigene und verbotene fremde CRUD-Nebenwirkungen. Neue Usage-Policy nur eigenes SELECT. [RLS-Suite](test/migrations/rls_cross_user.sql), [Privacy-Suite](test/migrations/privacy_deletion.sql). | Neue Policy nach Rollout mitsamt Grants/USING/WITH CHECK vergleichen; Tests bei jeder Schemaänderung erhalten. |
| A03 | Nutzer-/Mandantentrennung, Verknüpfungen | ERFÜLLT | P1 | TEST: tatsächliche eingeschränkte Rollen A/B, eigene CRUD erlaubt, fremde Reads/Änderungen ohne Wirkung; Sessions, Training, Rezepte und Mahlzeitkonvertierung eingeschlossen. LIVE: bisherige Definitionen entsprechen dem getesteten Stand. [RLS-Suite](test/migrations/rls_cross_user.sql). | Neue verknüpfte Domänen mit denselben Positiv-/Negativfällen absichern; neue Migrationen noch bereitstellen. |
...
| A05 | Zu offene Policies | ERFÜLLT | P1 | LIVE: keine USING(true)/WITH CHECK(true) in den 34 App-Policies, anon ohne App-Tabellengrants. TEST: neue Owner-Policy erkennt bewusst gelockerte Mutation als Fehler. [RLS](test/migrations/rls_cross_user.sql), [Usage-Policy](supabase/migrations/20260915093000_ai_usage_export.sql). | Erneuter vollständiger Policy-/Rollenabgleich nach kombiniertem Rollout. |
| A06 | Minimale Schema-/Tabellen-/Funktionsrechte | TEILWEISE | P2 | Zwei enge LIVE-Befunde: globales PUBLIC-EXECUTE-Default für künftige Funktionen und neun unnötige PG17-MAINTAIN-Grants. CODE/TEST: Korrektur rot→grün auf PG17.6 und PG16.15; bestehende App-RPCs bereits explizit begrenzt. [Migration](supabase/migrations/20260915090000_database_privilege_boundaries.sql), [Regression](test/migrations/database_privilege_boundaries.sql). | Migration als vorgesehener Owner bereitstellen, globale Defaults und verbliebene MAINTAIN-Grants zurücklesen. Kein aktueller beliebiger SQL-Endpunkt nachgewiesen. |
...
| A08 | RPCs, SECURITY DEFINER, search_path | ERFÜLLT | P1 | LIVE: alle 33 bisherigen App-Funktionen mit Quellstand, ACL, Owner und festem search_path abgeglichen; 15 Advisor-Warnungen einzeln bewertet. TEST: neue Budget-RPC nur privilegiert, atomar und begrenzt. [Migrationen](supabase/migrations), [RLS](test/migrations/rls_cross_user.sql), [Budgettests](test/migrations/ai_provider_budget.sql). | Neue RPC nach Rollout ebenfalls definitions-/grantseitig vergleichen; neue Funktionen benötigen explizites EXECUTE. |
...
| A11 | Bereitgestellte Policies/Grants/Advisor | TEILWEISE | P1 | LIVE: bisherige 43 Migrationen, 34 Policies, 36 Profilgrants, 72 Constraints, 16 FKs, 12 Trigger und 33 Funktionen verglichen; 18 Advisor-Einträge bewertet. Neuer lokaler Stand ist noch nicht live. [Schema](supabase/SCHEMA_STATE.md), [Migrationsquellen](supabase/migrations). | Nach Rollout tatsächliche Definitionen/Grants/Advisor erneut lesen; Versionsnummerngleichheit allein genügt nicht. |
...
| B03 | Echte serverseitige Tokenvalidierung | TEILWEISE | P2 | CODE/TEST: projektspezifisches /auth/v1/user prüft wirklich; GoTrue 2.196.0 mit gültigen A/B und ungültigen Tokens getestet. Neue Subject-/Audience-/Expiry-Anwesenheitsbindung rot→grün. Abweichender Issuer mit gültigem Projektschlüssel bleibt gesonderte Grenze. [Helper](supabase/functions/_shared/user_token_context.ts), [Probe](scripts/security/README.md). | Alle drei Bundles deployen; tatsächliche/Legacy-Issuer und bereitgestellte Authversion ohne echte User-JWTs belegen, dann zusätzliche Issuerbindung entscheiden. |
| B04 | Objekt-/Aktionsautorisierung jedes Endpoints | ERFÜLLT | P1 | CODE/TEST: alle drei Endpoints verfolgt. Coach bindet Session/History/Schreibwerte an bestätigte UID; A→B-Session erzeugt nur eigene Defaultsession. Analyse speichert keine Tagebuchdaten; Suche liefert öffentlichen Produktindex. [Handler](supabase/functions/coach-chat/handler.ts), [echte lokale Auth-Probe](scripts/security/README.md). | Neue Aktionen ebenso bis Daten/Nebenwirkungen prüfen; nach Deployment Quellenabgleich, kein Produktionstest mit fremden Daten. |
| B05 | Keine vertrauten Client-Behauptungen | ERFÜLLT | P1 | CODE/TEST: Identität nur aus verifizierter Authantwort; user_id/isAdmin/isPremium/role/model/messages verleihen keine Rechte. Neue Feld-Allowlist weist solche Bodies explizit zurück. [24 Feldregressionen](supabase/functions/_shared/request_fields_test.ts). | Neue Requestfelder serverseitig freigeben und Negativfälle beibehalten; geänderte Bundles bereitstellen. |
| B06 | Auth-Konfiguration aller Edge Functions | ERFÜLLT | P1 | LIVE: genau analyze-meal v30, coach-chat v47 und search-key v9, ACTIVE/verify_jwt=true; CODE zusätzlich eigene Authdienstprüfung. Keine alte öffentliche Function gefunden. [Backenddokumentation](docs/BACKEND.md), [Entrypoints](supabase/functions). | Nach dem noch offenen Drei-Function-Rollout Inventar, verify_jwt und Bundlequellen erneut prüfen. |
| B07 | Privilegierte Backend-Clients | ERFÜLLT | P1 | CODE/TEST: Servicezugriffe für Sessions/History/Titel nach Ownerprüfung; Limits aus bestätigter UUID; keine mutierende globale Requestidentität. A/B-Probe prüft Filter, Inserts und Sessionantwort. [Coach](supabase/functions/coach-chat/handler.ts), [Auth-Probe](scripts/security/README.md). | Neue privilegierte Aktionen und Budget-RPC nach Rollout mit Grant-/Definitionsevidenz verbinden. |
| B08 | Serverseitige Eingabevalidierung | ERFÜLLT | P2 | CODE/TEST: Coach acht, Analyse vier erlaubte Top-Level-Felder; JSON-Objekte, Typ-/Längen-/Trainings-/Bildgrenzen. Unbekannte Felder vor Session/Tagesquota/Provider abgewiesen; kompatible optionale Defaults bleiben. [Feldtests](supabase/functions/_shared/request_fields_test.ts), [Bildprüfung](supabase/functions/_shared/image_validation.ts). | Kombinierte Änderungen deployen; technische Requestgültigkeit nicht als fachliche/medizinische Richtigkeit bewerten. |
...
| ID | Prüfpunkt | Status | Priorität | Nachweis | Nächster Schritt |
|---|---|---|---|---|---|
| F01 | Eigene API-/AI-Limits | TEILWEISE | P1 | CODE/TEST: bestehende IP-/User-/Tagesgates plus neues unabhängiges Budget vor jedem tatsächlichen Provideraufruf; ungültige Bodies ohne Providerbuchung. [Budgethelper](supabase/functions/_shared/provider_budget.ts), [Coachtests](supabase/functions/coach-chat/handler_test.ts). | Migration vor beiden neuen AI-Bundles deployen, Quellen/Grants zurücklesen; bisherige Grenzen beweisen keine aktive neue Globalgrenze. |
| F02 | Nutzer-/globale Budgets und neue Accounts | TEILWEISE | P1 | CODE/TEST: atomar 1000 Calls/UTC-Tag global, davon 50 Bilder, 150/Konto; keine Providerbudget-Refunds. LIVE-App-Geldlimit unbekannt: geprüfter Vault-Key ist nicht Runtime-Key. [Migration](supabase/migrations/20260915091000_ai_provider_budgets.sql), [Races](test/migrations/ai_provider_budget_concurrency.py). | Appbudget ausrollen. Betreiber muss tatsächlichen Runtime-Key/Konto identifizieren und USD-Limit, Autoaufladung/BYOK prüfen; kein blindes Ersetzen des funktionierenden Keys. |
...
| F04 | Kostenprüfung und Verbuchung vor Ausführung | ERFÜLLT | P1 | CODE/TEST: Pflichtbudget vor Classifier, Chat/SSE, Rezept, Plan, Bild und Analyse. Sechs erstattete Fehler bei Callcap 3 erzeugen nur drei Calls. Kein Abo/Kaufpfad. [Helper](supabase/functions/_shared/provider_budget.ts), [Coachtests](supabase/functions/coach-chat/handler_test.ts). | Neue Implementierung live bereitstellen/verifizieren; neue Providerpfade dürfen den Pflichtparameter nicht umgehen. Kein USD-Reservierungsnachweis daraus. |
| F05 | Atomare Kontingente/Race Conditions | ERFÜLLT | P1 | TEST PG17.6: 20 Konten/Globalcap 7 → 7 erlaubt; zwölf parallele Calls/Usercap 4 → 4 erlaubt, gespeicherte Summen passend. Entfernte FOR-UPDATE-Sperre macht Regression rot. [RPC](supabase/migrations/20260915091000_ai_provider_budgets.sql), [Racetest](test/migrations/ai_provider_budget_concurrency.py). | Rollout/Grants vergleichen. Alte Fragenrefunds ohne Reservation-ID nicht als allgemeine Exactly-once-Garantie darstellen; neues Callbudget wird nie erstattet. |
...
| F07 | Alarme und serverseitige Notabschaltung | TEILWEISE | P1 | CODE/TEST: globale/funktionsbezogene DB-Schalter vor jedem Call, fehlende Konfiguration scheitert geschlossen; noch nicht live. Alarmzustellung nicht belegt. [Budgettests](test/migrations/ai_provider_budget.sql), [Notfallablauf](docs/OPERATIONS.md). | Budget-Rollout abschließen; Operator/Alarmroute benennen und abgeschirmten Regel-/Zustellnachweis planen. Schalter beendet keine bereits gestartete Providerarbeit. |
...
| G03 | Vertrauenswürdige Rollen/Modellparameter | ERFÜLLT | P1 | CODE/TEST: Systemrollen/Modelle serverseitig; History nur user/assistant; Client kann keine Rollen-, Tool- oder Modellliste einschleusen. Neue Feldtests weisen Manipulation ab. [Handler](supabase/functions/coach-chat/handler.ts), [Feldregression](supabase/functions/_shared/request_fields_test.ts). | Geänderte Bundles und echte Konfiguration nach Rollout vergleichen; neue Rollenparameter nur serverseitig zulassen. |
...
| I03 | Einwilligungen und zutreffende Information | TEILWEISE | P1 | CODE/TEST: DE/EN-Coachinformation korrigiert, ursprüngliche Websitekorrektur bereits veröffentlicht. Neue Usage-/ereignisabhängige Retentionsangaben vorbereitet. [Datenschutz](PRIVACY.md), [Disclosuretest](test/coach_ai_disclosure_test.dart), [Websitekorrektur](docs/PRIVACY-WEBSITE-CORRECTION-2026-09-14.md). | Neue Angaben passend zum tatsächlichen Rollout veröffentlichen; Rechtsgrundlagen, Gesundheitsdaten/Einwilligungsdesign und Verträge fachlich prüfen lassen. |
...
| K03 | Bekannte Schwachstellen der Abhängigkeiten | TEILWEISE | P1 | TEST: 153 Pub-, neun Swift-Revisions- und 169 Android-Runtimekomponenten ohne OSV-Match. Gradle-8.14-Resolverlücke mit Herstellerbeleg rot→grün auf 8.14.4; 45 separate Buildtool-Advisories nach Erreichbarkeit triagiert. [Wrapper](android/gradle/wrapper/gradle-wrapper.properties), [Buildtool-Triage](android/BUILD_TOOL_SECURITY.md), [Inventar](tool/security_inventory.py). | Finales integriertes Build/CI prüfen; verbleibende passende Buildtool-Updates planen. OSV-zero beweist keine vollständige Schwachstellenfreiheit. |
| K04 | Secret-/statische-/RLS-Prüfungen in CI | TEILWEISE | P1 | CODE/TEST: erforderliche Checks erhalten; native/Swift-Inventare, PG17.6, Budget-Races, Restore und Offline-Eval integriert/lokal geprüft. LIVE: Secret-/Pushschutz und acht Checkbindungen an GitHub Actions. [CI](.github/workflows/security.yml), [RLS](test/migrations/rls_cross_user.sql), [Restore](test/operations/backup_restore.py). | Exakt getesteten PR-Head nach grünem vollständigem CI und aktuellem main prüfen. Driftjob vergleicht nur Migrationsnummern. |
...
| K08 | Rotation/Sperre/Rollback/Notabschaltung | TEILWEISE | P1 | CODE/TEST: Runbook und synthetischer Restore vorhanden; AI-Schalter nach noch offenem Rollout verfügbar. LIVE: Private Vulnerability Reporting aktiviert/zurückgelesen. [Notfallablauf](docs/OPERATIONS.md), [Budgetschalter](supabase/migrations/20260915091000_ai_provider_budgets.sql). | Verantwortliche benennen und Rotation/Sperre/Rollback/Alarm im Tabletop üben; kein Schlüssel ohne geklärte Zuordnung ersetzen, kein Appupdate als einzige Abwehr. |
...
| L02 | CORS und gegebenenfalls Cookie-CSRF | TEILWEISE | P2 | CODE/TEST: 48 Antworten über drei Handler/acht Origins; Coach-Origin auf echten JSON/SSE-Antworten korrigiert, keine Credentials-CORS. LIVE: Origin-Allowlist derzeit nicht gesetzt; Bearerauth statt Cookies. [Matrix](supabase/functions/_shared/cors_origin_contract_test.ts), [Coachtests](supabase/functions/coach-chat/handler_cors_test.ts). | Fix deployen/verifizieren; keine Allowlist ohne Webbedarf aktivieren. CORS ist keine Objekt-/Tokenautorisierung. |
...
- **Rest:** Defaults anderer Owner, darunter `supabase_admin`, nicht global umgebaut. DDL-/ACL-Liveabgleich nach Root-Rollout erforderlich; Repository-Migration allein ist dafür kein Beleg.
...
- **Auswirkung/Rest:** Echte App-Abhängigkeitsauflösung mit AGP 8.11.1/Kotlin 2.2.20/Java 21 geprüft; endgültigen Release-/R8-/AOT-Build separat nachweisen. OSV kannte diese Gradle-Advisories nicht: null Scannertreffer waren kein Entwarnungsbeweis.
