...
Stand: **16.09.2026**. Runde 4 ergänzt den vollständigen 91-Punkte-Audit um fünf gezielte Fachprüfungen, Korrekturen und Gegenreviews. Die Tabelle enthält den aktuellen Nachweisstand. Frühere Runden bleiben datiert erhalten; eine Codekorrektur ist keine pauschale Sicherheitsfreigabe.

## Runde 4 — Nacharbeit mit fünf Agents

Die nächtliche Runde 3 wurde regulär abgeschlossen: PR #92 war am 15.09. um **02:49:43 MESZ** gemergt, die Session um **02:51:48 MESZ** beendet. Alle elf Main-Checks wurden grün, der letzte um **03:09:17 MESZ**. Der geplante Windows-Update-Neustart folgte erst um **04:29:06 MESZ**. Ein Abbruch dieses Audits durch das Update ist damit nicht belegt.

Die autorisierte Nacharbeit erfolgt isoliert ab Main `bc8957b`; fremde Änderungen im ursprünglichen Arbeitsordner bleiben erhalten. Genau fünf Agents bearbeiten Auth, Medien, Coach, Abhängigkeiten und Betrieb; Root integriert den Exportfix und die gemeinsame Prüfung. Datenbankverhaltenstests bleiben synthetisch/lokal. Keine produktiven Nutzer- oder Gesundheitsdaten, echten Provider-Generierungen oder Alarmnachrichten werden für diese Runde verwendet.

### Behobene und weiter geprüfte Punkte

| Bereich | Änderung und tatsächlicher Nachweis | Verbleibende Grenze |
| --- | --- | --- |
| Eigenexport · I05 | Einträge werden über den letzten Zeitstempel und die ID weitergelesen. Das Löschen einer schon exportierten Zeile konnte beim bisherigen Offsetverfahren die nächste ungelesene Zeile überspringen. Der Fehler wurde reproduziert; Regressionen für Löschen, Einfügen, gleiche Zeitstempel und Mikrosekunden bestehen. [Exportregression](test/services/data_export_concurrency_test.dart). | Kein atomarer Datenbanksnapshot: gleichzeitige Änderungen am Sortierzeitstempel können Zeilen verschieben. Lokale/Anbieter-/Backupdaten bleiben gesonderte Auskunftsarbeit. |
| Coach · F02/G/K06 | Ein Clientabbruch erstattet bereits verbrauchte Tagesfragen auch in Rezept-, Plan-, Classifier- und Persistenzfehlerpfaden nicht mehr. Reguläre Infrastrukturfehler behalten ihre Erstattung; fertig gepufferte Vorschläge bleiben wie vorgesehen speicherbar. Rohe Transport-/Speicherfehler gelangen nicht in Diagnoselogs. Acht reproduzierte Fehlfälle vor der Änderung; **35 neue Handlerfälle** mit kontrollierten Providerantworten. [Regressionen](supabase/functions/coach-chat/handler_boundary_test.ts). | Kontrollierte Antworten belegen technische Grenzen, keine semantische Bild-/Gesundheitssicherheit eines realen Modells. Geldbudget und Alarmzustellung bleiben Betreiberaufgaben. |
| Bilder · D04/D05 | Client und Server weisen auch verwaiste PNG-Animationsblöcke `fcTL`/`fdAT` ab. Android-Profilbuild mit AOT verarbeitet vier synthetische Bildfälle über den produktiven Decoderpfad erfolgreich. [Prüfaufbau und Messungen](scripts/security/PHOTO_DECODER_PROBE.md). | Prozessspitzen bis **465,1 MiB**, längster Fall **7,195 s**. Die 64-MiB-Rastergrenze ist kein Prozessspeicherlimit. Kein Worst-Case-, physischer Geräte-, iOS- oder Store-Nachweis. |
| Auth · C01/C04/C07 | **104 echte lokale GoTrue-Assertions** zu Refreshrotation/Replay, Logout, Passwort-/Recovery-/E-Mail-Wechsel, Sperre und Löschung; **47 Handler-/Authfälle**, Providerstubs mit vorherigen Budgetreservierungen. Bewusst deaktivierte Rotation wird erkannt. Der Probe blockiert Proxys/Redirects und wartet auf den endgültigen PostgreSQL-Server. [Ablauf](scripts/security/README.md). | Keine Aussage über gehostete Gateway-Header, echte SMTP-/Google-Zustellung oder installierte Endgeräte. |
| Buildwerkzeuge · K03/K04 | Jetifier/KAPT bleiben mit realen Gradle-Regressionen deaktiviert; Aktivierungen über CLI/Systemeigenschaften scheitern vor Tasks. Ein tatsächlicher AGP-8.13.2-Vergleich behebt keine der **45 Advisory-IDs**, deshalb kein wirkungsloses Versionsupdate. Debug-APK sowie Release-AAB/R8/AOT mit Dummy-Konfiguration bestehen; **169** Runtimekomponenten ohne OSV-Treffer. [Datiertes Triageergebnis](android/BUILD_TOOL_SECURITY.md). | **152 Buildwerkzeugkomponenten / 45 IDs / 46 Treffer** bleiben sichtbar. Die Policy ist kein Sandboxschutz gegen beliebigen Gradle-Code; passende Herstellerupdates bleiben notwendig. |
| Betriebsprüfung · K05/K07/F02 | Neues ausschließlich lesendes Skript für richtige Projektidentität, Backupalter und aggregierte KI-Budgets; **26 Tests** einschließlich unbekannter/staler Zustände und drei nachgewiesenen Fehlmutationen. Die echte SQL-Abfrage läuft auf allen 46 Migrationen im isolierten PostgreSQL. [Checker](scripts/operations/readiness.py), [Betriebsablauf](docs/OPERATIONS.md). | Kein installierter Zeitplan, Alarmempfänger, Geldlimit oder produktiver Restore. Das Skript prüft diese Grenzen ausdrücklich getrennt. |

**Live-Abgleich, 15.09.2026 21:45 UTC:** Nach anfänglichem HTTP 503 war die Management-API wieder erreichbar. Die [angekündigte Plattformwartung](https://status.supabase.com/incidents/79k7dh48kvh7) lag im selben Zeitfenster; sie ist eine plausible Erklärung, kein bewiesener Kausalnachweis. Projektidentität und `ACTIVE_HEALTHY` sind bestätigt. Kein abgeschlossener Backup-Wiederherstellungspunkt ist nachgewiesen; PITR ist aus, kein physisches Restorefenster vorhanden. Alle vier KI-Schalter sind aktiv; Limits bleiben 1000 globale Calls, 150 je Konto und 50 Bilder pro UTC-Tag. Aggregierter aktueller Tagesverbrauch beim Abgleich: null. Geldlimits werden dadurch nicht nachgewiesen. [Sanitisierter Lesebericht](docs/SECURITY-READINESS-2026-09-15.json).

**Gemeinsame Prüfung:** 700 Deno-/Offline-Evaltests und alle 36 Function-Testdateien einzeln bestehen; alle drei Entry Points sind typgeprüft, Lint ist grün. PostgreSQL 17.6 spielt alle 46 Migrationen ab und besteht RLS-/Lösch-/Budgetprüfungen. Flutter 3.47.2/Dart 3.13.2 besteht die strikte Analyse und alle **4626 Tests**; Zeilenabdeckung **95,03%** (Floor 88%, generierte Lokalisierung ausgenommen). Die Auth-, Betriebs- und Build-Policy-Regressionen sind an bestehende CI-Jobs angeschlossen. Unabhängige Gegenreviews fanden Probeprobleme beim Datenbankstart und in der Proxy-/Redirect-Isolierung (Python und Deno), die korrigiert und erneut geprüft wurden; keine offene Codeblockade im überprüften Export-/Coach-/Medien-/Gradle-Diff.

**Auslieferung:** Nach grüner [PR-CI](https://github.com/mxritzgit/Eatova/actions/runs/35028139431) auf Quellstand `236f68be21d3f2e2c5f8c4a6214ed4affb0b109f` wurden `coach-chat` **v49** und `analyze-meal` **v32** bereitgestellt und vollständig zurückgelesen: ACTIVE, `verify_jwt=true`, alle importierten TypeScript-Quelltexte identisch mit dem getesteten Stand. Die bisherigen v48/v31-Quellen sind als Rückfallkopien gesichert. `search-key` bleibt v10; keine neue Migration oder Secretänderung. Modellkonfiguration und Secret-Fingerprints wurden ausschließlich im Prozess erneut verglichen. Abschluss **2026-09-15T22:16:37.599024+00:00**. [Sanitisierter Rolloutnachweis](docs/SECURITY-ROLLOUT-FOLLOWUP-2026-09-15.json); [PR #93](https://github.com/mxritzgit/Eatova/pull/93) dokumentiert den geschützten Merge und die finalen Checks. Eine neue Nutzer-App ist nicht installiert.

### Was der Betreiber noch erledigen muss

1. **MFA:** Supabase-Owner-Faktor und Wiederherstellung einrichten; GitHub-Owner-MFA in den angemeldeten Kontoeinstellungen prüfen.
2. **Backups:** Zulässigen Datenverlust (RPO), Wiederherstellungszeit (RTO), verschlüsseltes Ziel, Zeitplan, Aufbewahrung und Verantwortliche festlegen. Einen echten Wiederherstellungspunkt einrichten und in ein getrenntes Ziel zurückspielen; Auth/gegebenenfalls Storage berücksichtigen.
3. **Anbieter und Alarme:** Tatsächlich bereitgestellten OpenRouter-Key dem richtigen Konto zuordnen, Geldlimit/Intervall/Autoaufladung und Datenschutzoptionen prüfen. Empfänger und Vertretung benennen, Checker sicher einplanen, Ausfall-/Fehler-/Budgetalarme samt Zustellung testen. Aufrufgrenzen ersetzen kein Geldlimit.
4. **Staging und Geräte:** Eigenes Testprojekt mit getrennten Credentials/Budgets bestimmen. Google-/SMTP-/Gatewayabläufe dort sowie signierte Builds auf physischen Android-/iOS-Geräten prüfen; anschließend Appupdate ausliefern. Die gemessenen Decoder-Ressourcen auf kleineren Geräten bewerten.
5. **Fachliche Entscheidungen:** Gesundheits-/Minderjährigen-/Verletzungsfälle und echte Modell-Bildangriffe fachkundig abnehmen. Aufbewahrung, Datenschutzverträge, externe Export-/Löschabläufe und mögliche Fristausnahmen festlegen. Garantierte zeitgesteuerte Löschung ist weiterhin nicht eingerichtet; aktuelle Bereinigung hängt von Aktivität ab.

Die 91 Statuswerte bleiben bewusst bei **30 ERFÜLLT, 46 TEILWEISE, 2 NICHT PRÜFBAR, 13 NICHT RELEVANT**. Zusätzliche technische Nachweise schließen die verbleibenden Betreiber-, Plattform- und fachlichen Grenzen nicht automatisch.

## Runde 3 — Abschlussstand (vor Runde 4)
...
Aktueller Stand der 91 Prüfpunkte nach Runde 4. CODE, TEST und LIVE sind getrennte Nachweise; ERFÜLLT gilt ausschließlich für den genannten Umfang. Prioritäten offener Nachweise sind Prüfprioritäten, keine bewiesenen Schwachstellen.
...
| ID | Prüfpunkt | Status | Priorität | Nachweis | Nächster Schritt |
|---|---|---|---|---|---|
| C01 | Registrierung/E-Mail/Passwortschutz | TEILWEISE | P2 | LIVE: E-Mail-Bestätigung nötig, Passwortminimum 8, OTP acht Ziffern/600 s; HIBP aus, Freeplan. CODE/TEST: passende Authabläufe. [Repository](lib/src/auth/auth_repository.dart), [Signup-/OTP-Tests](test/auth_otp_session_isolation_test.dart). R4 TEST: lokales GoTrue lehnt zu kurze Passwörter ab und verlangt verifizierte Signup-Codes. | Kompromittierte-Passwort-Prüfung und nötigen Tarif bewusst entscheiden; SMTP-Zustellung und gehostete Policy in separater Testumgebung prüfen. |
...
| C04 | Sessionablauf/Refresh/Logout/Sperren | TEILWEISE | P2 | LIVE: JWT 3600 s, Refreshrotation/10-s-Reuse; keine absolute/Idle-Grenze. TEST: spätes OTP stellt nach Logout keine A-Sitzung wieder her; Android-Persistenz geprüft. [OTP-Tests](test/auth_otp_session_isolation_test.dart), [Storage-Tests](test/services/session_logout_restore_test.dart). R4 TEST: echte lokale Rotation, Replay-Familiensperre, lokaler/globaler Logout sowie gebannte/gelöschte Konten geprüft. | Gehosteten Gateway-/Client-Roundtrip in eigener Staging-Umgebung prüfen. Bereits ausgestellte Access-JWTs nicht als durch lokalen Logout sofort ungültig darstellen. |
...
| C07 | Accountverknüpfung/Wiederherstellung | TEILWEISE | P1 | LIVE: manuelles Linking/Anonymous aus, Google an, Apple aus. TEST: bewiesener später Recovery-/Signup-Sessionwechsel auf A behoben, zulässiger Login nach Logout/Refresh erhalten. [Mutationsguard](lib/src/auth/auth_session_mutation.dart), [A/B-Regression](test/auth_mutation_session_isolation_test.dart). R4 TEST: doppelte E-Mail-Änderung und Recovery-Bindung/Replay/Expiry mit echtem lokalen Authserver geprüft. | Google-Verknüpfung und echte Mailzustellung im Testprojekt prüfen; Clientfix noch an Endgeräte ausliefern. |
...
| D04 | Uploadgröße und tatsächlicher Dateityp | TEILWEISE | P2 | CODE/TEST: kanonisches Base64, Containerstruktur und Rasterbudget vor Session/Provider; gemessener Bildtyp statt Client-MIME. Analyse 5 MB, Coach 6 Mio Base64-Zeichen. [Servervalidator](supabase/functions/_shared/image_validation.ts), [Tests](supabase/functions/_shared/image_validation_test.ts). R4: verwaiste PNG-Animationsblöcke werden ebenfalls verworfen. | Beide AI-Funktionen mit Runde-4-Quellen deployt und vollständig zurückgelesen; Strukturprüfung ist kein vollständiges serverseitiges Pixeldecoding. Keine Live-Bildprobe ausgeführt. |
| D05 | Bildverarbeitung/Dimensionen/EXIF | TEILWEISE | P2 | CODE/TEST: Metadaten-Originalpfad repariert, EXIF vor Decoder entfernt; 64-MiB-RGBA-/32-MiB-Encoded-Grenzen, beschränkte PNG-/ICC-Inflation, validierter JPEG-Ausgang. [Container](lib/src/services/photo_container.dart), [Metadatenregression](test/services/meal_photo_metadata_boundary_test.dart). R4 TEST: vier Android-AOT-Decoderfälle; Prozesspeak bis 465,1 MiB, siehe [Messbericht](scripts/security/PHOTO_DECODER_PROBE.md). | Physische Geräte, kleinere RAM-Budgets und iOS prüfen; Rasterbudget ist kein Prozessspeicherlimit. Direkte API-Bilder werden serverseitig nicht neu kodiert; Provider-Metadaten bleiben möglich. |
...
| G08 | Nutzertrennung in Chat/Memory/Caches | TEILWEISE | P1 | CODE/TEST: Historyfilter UID UND Session, Rollen-/Längengrenzen; keine Embeddings/Vektorsuche/Antwortcaches. A/B-SQL/Endpoint- und lokale Geräte-/Cleanup-Proben grün. [Handler](supabase/functions/coach-chat/handler.ts), [RLS](test/migrations/rls_cross_user.sql), [Accountwechsel](test/coach_session_race_test.dart). R4 TEST: gleichzeitige A/B-Handlerfälle prüfen getrennte History und Persistenz mit Providerstubs. | Integrierten signierten Client und isolierten Cloud-A/B-Roundtrip ergänzen; kein Produktionschat für Prüfung gelesen. |
...
| K05 | DB-/Storage-Backup und Restore-Nachweis | TEILWEISE | P1 | TEST: zwei getrennte PG17.6-Cluster, synthetischer Restore mit Daten-/Katalog-/Rollenvergleich und A/B-Zugriff; RLS-/Datenmutanten erkannt. LIVE: Free, PITR aus, Backup-Liste/Restorefenster leer. [Rehearsal](test/operations/backup_restore.py), [Runbook](docs/OPERATIONS.md). R4 LIVE: am 15.09. 21:45 UTC erneut kein abgeschlossener Backup-Wiederherstellungspunkt nachgewiesen; der neue Freshness-Checker meldet Handlungsbedarf. | Produktionsbackupziel, Zeitplan, Retention, RPO/RTO und tatsächlichen nutzbaren Restorepunkt festlegen/nachweisen; lokaler Restore ist kein Produktionsrestore. |
| K06 | Nachvollziehbare, sparsame Sicherheitslogs | TEILWEISE | P1 | CODE/TEST: Sentry- und Providerdiagnostik gefiltert, keine unnötigen Inhalte; erlaubte technische Felder geprüft. LIVE-Admin-Auditretention unzugänglich, Logdrains 403, kein Sentry-Adminzugang. [Sentry](lib/src/services/crash_reporter.dart), [Providerlogtests](supabase/functions/_shared/provider_log_test.ts), [Betrieb](docs/OPERATIONS.md). R4: rohe Providertransport-/Speicherexceptions durch feste Diagnosekategorien ersetzt und negativ getestet. | Audit-/Sentry-Einstellungen mit nötigem Lesezugriff prüfen; keine echten Health-/Logpayloads für den Nachweis exportieren. |
| K07 | Alarme für Kosten/Fehler/Datenmissbrauch | NICHT PRÜFBAR | P1 | CODE/DOKU: konkrete begrenzte Alarmvorschläge und sichere Prüfschritte im Runbook. Kein zugänglicher Nachweis tatsächlicher Regeln, Empfänger oder Zustellung; kein Testalarm versendet. [Monitoringablauf](docs/OPERATIONS.md). R4 CODE/TEST: lesender Budget-/Backup-Checker mit getrennten attention/unknown-Zuständen vorbereitet; nicht eingeplant. | Provider-Admin-Lesezugang, Haupt-/Vertretungszuständigkeit und autorisierte isolierte Zustellprobe organisieren; fehlender Nachweis bedeutet nicht automatisch keine Alarme. |
...
Die folgenden Aussagen der Runden 1 und 2 beschreiben den damaligen Stand. Für aktuelle Erfüllung, Live-Nachweise und verbleibende Aufgaben gelten Runde 4 und die aktuelle Tabelle. Insbesondere sind alte Versions-, Migrations- und Testzahlen keine aktuellen Gesamtzahlen.
