...
from ios_test_support import SHARE_SUITES, select_device, verify_results
...
def result_tree():
    return {"testNodes": [{"nodeType": "Unit test bundle", "children": [
        {"nodeType": "Test Suite", "name": suite, "children": [
            {"nodeType": "Test Case", "nodeIdentifier": f"{suite}/testWorks()", "result": "Passed"},
...
class ResultVerificationTests(unittest.TestCase):
    def test_required_suites_have_executed_passed_tests(self):
        self.assertEqual(verify_results(result_tree()), dict.fromkeys(SHARE_SUITES, 1))
    ...
    def test_nested_destination_and_module_qualified_suite_names(self):
        ...
        self.assertEqual(verify_results({"testNodes": [{"nodeType": "Destination", "children": tree["testNodes"]}]}), dict.fromkeys(SHARE_SUITES, 1))

    def test_missing_empty_or_skipped_suite_fails_instead_of_false_green(self):
        baseline = result_tree()
        for mutation in ("missing", "empty", "skipped", "failed", "unknown"):
