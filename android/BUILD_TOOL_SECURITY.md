...
The actual configured projects have no KAPT plugin/task. Jetifier is unset in
the repository and defaults to false in this AGP version. The generated SBOM
records these assumptions for each CI run. Normal CI requests assemble/bundle,
...
| JDOM2.0.6 via Jetifier1.0.0-beta10 | Jetifier constructs default SAXBuilder and parses POM XML without explicit entity restrictions. This is a risky path if Jetifier is enabled and handles attacker-influenced XML. Current AGP defaults Jetifier off; external flag overrides must be reconsidered. | Keep Jetifier off unless required. The indexed2.0.6.1 patch alone does not make a default SAXBuilder universally XXE-safe; enabling this path requires caller parser hardening plus benign-POM/XXE tests. [Supplier release](https://github.com/hunterhacker/jdom/releases/tag/JDOM-2.0.6.1). |

Prefer a compatible vendor AGP patch/minor whose **resolved** classpath improves
these dependencies. No such upgrade has yet been validated for this whole set.
